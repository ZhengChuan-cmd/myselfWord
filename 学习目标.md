...
6.事务与隔离级别
	ACID特性
	隔离级别：读取未提交、读取已提交、可重复读、串行化
	并发问题：脏读、不可重复读、幻读
	MVCC（多版本并发控制原理（InnoDB实现）
...
```
1.常用命令（文件，文本，权限）
	文件操作：ls -all、cd、pwd、mkdir -p、rm -rf、cp、mv、touch、ln -s
	查看文件：cat、more、less 、head、tail  -f
	文本处理：grep、sed、awk、sort、uniq、wc、cut
	权限管理：chmod（数字/符号）、chown、chgrp、umask、SUID、/SGID/sticky bit
	查找：find、locate、which、whereis

2.进程管理
	查看进程：ps aux、ps -ef、top、htop、pstree
	进程信号：kill -9 (SICKILL)、kill -15(SIGTERM)、kill -3(SIGQUIT 打印线程栈)
	作业控制：&（后台运行）、jobs、fg、bg、nohup
	守护进程：systemctl(systemd)、service
	
3.网络相关：
	查看端口：netstat -tunlp、ss -tunlp、lsof -i:端口
	测试连通：ping、telnet、nc、curl
	路由：route -n、ip route
	抓包：tcpdump、wireshark(基础过滤)
	
4.磁盘与内存
	磁盘：df -h、du -sh、fdisk、mount
	内存：free -h、vmstat、top中的RES/VIRT/SHR
	IO：iostat、iotop

5.系统管理与监控
	查看系统信息：uname -a、cat /etc/os-release、uptime
	查看日志：dmesg、tail -f /ver/log/mssage（或/var/log/syslog）
	性能排查：top、htop、vmstat 1、iostat -x 1、sar、netstat -i
	查看文件描述符：ulimit -a、lsof
...
7.常见服务与Java运维
	jdk安装与环境变量：JAVA_HOME、PATH、CLASSPATH
	查看java进程：jps、ps -ef|grep java
	线程栈：jstack、kill -3
	内存堆转储：jmap -dump:liv、format=b、file=heap.bin <pid>
	CG日志分析：jstat -gutil
	
10.问题：
	grep 和 find 的区别是什么？
	答：grep 在文件内容中搜索匹配文本；find 根据文件名、大小、时间等属性搜索文件。

	2. nohup 命令的作用是什么？
	答：让命令在后台运行，并且忽略挂断信号（SIGHUP），即关闭终端后进程仍继续运行。常用于启动长期服务：nohup java -jar app.jar &。

	3. 如何快速定位某个端口（如 8080）被哪个进程占用？
	答：netstat -tunlp | grep 8080 或 lsof -i:8080，得到 PID 后用 ps -ef | grep PID 查看详情。

	4. 解释 ps aux 输出中的 STAT 列（如 R、S、D、Z、T）。
	R：运行中或可运行。
	S：可中断睡眠（等待事件）。
	D：不可中断睡眠（通常为 IO）。
	Z：僵尸进程（已结束但未被父进程回收）。
...
5. 如何查看 Linux 系统的平均负载（load average）？
	答：uptime、top、cat /proc/loadavg。三个数值分别表示过去 1、5、15 分钟的平均活跃进程数（包括正在运行和不可中断睡眠的进程）。

6. 写出一个 Shell 脚本，循环打印当前时间，每秒一次，共 10 次。
...
for i in {1..10}; do
	...
	7. 如何将当前目录下所有 .log 文件中的 "ERROR" 行输出到一个汇总文件中？
	答：grep "ERROR" *.log > error_summary.txt
```
...
1.IoC（控制反转） 与 DI（依赖注入）
	IoC:将对象的创建、组装、管理权限从应用程序代码转移到Spring容器
	DI:IoC的具体实现方式，容器在创建Bean时动态的将依赖注入进去
	注入方式：
		构造器注入（推荐，保证不可变性）
		Setter注入
		字段注入（@Autowrired，不推荐，不利于单元测试）

2.Spring Bean
	Bean定义：有@Bean 或 @Component 等标注的对象，有Spring容器管理
	Bean名称：默认类名首字母小写，可指定 @Component("customName")
	Bean作用域（Scope）:
		singletion（默认）：容器只有一个实例
		prototype：每次获取都创建新实例
		request（Web）：每个Http请求一个实例
		session（Web）:每个HTTP Session一个实例
		application（Web）:每个ServletContext一个实例
	Bean生命周期：
		实例化->属性赋值->初始化前（BeanPostProcessor.postProcessBeforeInitialization->初始化（@PostConstruct 或 afterPropertiesSet）->初始化后（BeanPostProcessor.postProcessAfterInitialization）->使用->销毁（@PreDestory 或 disposableBean.destory）
	
3.AOP（面向切面编程）
	概念：将横切关注点（日志，事务，安全等）与业务逻辑分离
	核心术语：
		Aspect（切面）：横切关注点的模块化（如 @Aspect）
		Join Point（连接点）：方法执行、异常抛出等可被拦截的点
		Advice（通知）：切面在连接点上执行的动作（@Before、@After、@Around、@AfterReturning、@AfterThrowing）
		Pointcut（切入点）：一组连接点的集合，定义何时执行通知
		Weaving（织入）：将切面应用到目标对象的过程（编译期、类加载期、运行期）
	Spring AOP实现：基于JDK动态代理（接口）或CGLIB（子类）
```

## 2.Spring配置方式演进

```
1.XML配置（传统）：<bean id ="..." class ="...">

2.注解配置（Spring 2.5+）：
	@Component/@Service/@Repository/@Controller
	@Autowrited/@Resource/@Inject
	@Value（注入配置文件属性）

3.Java Config（Spring 3.0+）
	@Configuration + @Bean
...
```
编程式事务：TransactionTemplate，不常用
声明式事务：@Transactional（推荐）
事务传播行为（Propagation）:
	REQUIRED（默认）：当前有事务则加入，无则新建
	REQUIRES_NEW：挂起当前事务，新建事务
	SUPPORTS：有事务则加入，无则以非事务执行
	NOT_SUPPORTED:非事务执行，挂起当前事务
	MANDATORY:必须非事务中运行，否则抛异常
	NEVER:必须在已有事务中运行，否则抛异常
	NESTED:嵌套事务（保存点机制，部分回滚）
隔离级别：参考SQL隔离级别（DEFAULT使用数据库默认）
事务失效场景：
	@Transactional 加在非public方法上（Spring要求必须时public）
	同类中方法调用（AOP代理不生效），需要通过代理对象调用
	异常被catch未抛出，或抛出的不是RuntimeException（可指定 rollbackFor）
	数据库引擎不支持事务（如 MyISAM）
```
...
核心组件：
	...
	HandlerMapping:映射请求到控制器方法
	Controller:处理请求，返回ModelAndView
	ViewResolver:解析视图（JSP、Thymeleaf等）
...
RESTful:@RestController = @Controller + @ResponseBody
```

## 5.Spring 常用注解总结

```
@Component		声明一个普通Bean
@Service		标识Service层Bean
@Respository	标志DAO层Bean（自动转换持久化层异常）
@Controller		标识控制器层Bean
@Autowired		按类型自动注入
@Qualifier		按名称注入（配合@Autowrited）
